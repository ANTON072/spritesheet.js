    ...
    function(file, next) {
      ...
      exec(
        'convert' +
          ...
          file.originalPath +
          '" -bordercolor transparent -border 1 -trim "' +
          file.path +
          '"',
        next
      );
...
exports.generateImage = function(files, options, callback) {
  var command = [
    'convert -define png:exclude-chunks=date -quality 0% -size ' +
      options.width +
      'x' +
      options.height +
      ' xc:none'
...
exports.generateData = function(files, options, callback) {
  var formats = (Array.isArray(options.customFormat)
    ...
    : [options.customFormat]
  ).concat(Array.isArray(options.format) ? options.format : [options.format]);
  formats.forEach(function(format, i) {
    if (!format) return;
    var path =
      typeof format === 'string'
        ? format
        : __dirname + '/../templates/' + format.template;
    ...
    options.files.forEach(function(item, i) {
      if (format.template === 'cssrem.template') {
        item.spritesheetWidth = options.width / 10 / 2;
        item.spritesheetHeight = options.height / 10 / 2;
        item.width -= options.padding * 2;
        item.height -= options.padding * 2;
        item.width = item.width / 10 / 2;
        item.height = item.height / 10 / 2;
      ...
      if (item.trim) {
        ...
        item.trim.offsetX = Math.floor(
          Math.abs(item.trim.x + item.width / 2 - item.trim.width / 2)
        );
        item.trim.offsetY = Math.floor(
          Math.abs(item.trim.y + item.height / 2 - item.trim.height / 2)
        );
      }
